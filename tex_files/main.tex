...
computing the adversarial network costs substantially. \cite{Jin2020} Jin et al. propose to address this issue by perturbing the latent 
...
to perturbations of the node attributes. If a node has been certified with their method, it is guaranteed to be robust under any possible 
perturbation given the attack model. Likewise, they can certify non-robustness. The proposed semi-supervised training procedure that treats 
...
Therefore, the authors introduce a global perturbation budget $Q \in \mathbb{N}$ as well as a local budget $q \in \mathbb{N}$.
...
\begin{figure}[h]
    ...
    \caption{Before perturbation \cite{Z_gner_2019}}
...
\begin{figure}[h]
    ...
    \caption{After perturbation \cite{Z_gner_2019}}
...
To make the objective function convex, they have to find a convex relaxation of the ReLU activation function. There are several ways to achieve this
...
crucial to obtain robustness certificates, as tighter bounds lead to lower relaxation error of the GNN activations. Futhermore, the bounds can 
...
\end{multline}
The authors conclude that since the dual program and the lower and upper activation bounds are differentiable, a robust GNN can be 
...
of GNNs while sacrificing only minimal accuracy on the unlabeled nodes. On average, the labeled nodes are more robust than the unlabeleld
ones, which is not surprising. The authors highlight two important observations:
\begin{enumerate}
...
The authors see the difficulty here in the fact that the message passing matrix is used at each layer, as opposed to the input features,
...
across the whole graph. The authors assume an attacker has potentially inserted edges to the unknown original graph 
...
However, as the authors claim, any optimization problem involving $\tilde{A}$ in the objective function is not convex 
in the variables $\tilde{A}_{ij}$, which means that a simple continuous relaxation does not leat to a tractable optimization problem.
...
The authors carefully design the set of admissible degree-normalized message passing matrices $\mathcal{\hat{A}}(A)$ based
...
The authors highlight that it is not necessary to find the global solution of the bilinear problem, 
...
stop as soon as possible. If the lower bound is positive, they successfully certified robustness. If the upper bound is negative, 
no robustness certificate can be given for the instance, which does not imply that the classifier is not robust, 
it just can not be certified. They further improve the procedure by exploiting knowledge about the graph domain.\newline
...
To evaluate the method on different severities of perturbations, the authors use three different local ($q$) and global ($Q$) perturbation budgets. 
Their method can certify robustness, but not non-robustness. Although obtaining the number of non-robust nodes is intractable, they
...
The general trend they observe is that increasing the budget decreases the number of certifiably robust nodes and increases
...
The latest work on certifiable robustness of GNNs against adversarial perturbations is provided by Wang et al. \cite{wang2020certified}.
The authors prove the first certified robustness guarantee of any GNN for both node and graph classifications.
...
a robust one via adding random noise to a testing example auch as a graph. A classifier is considered to be certifiably robust in their method
...
they assume testing examples to be continuous although graphs are binary data structures.
...
In summary, the authors prove the first certified robustness guarantee of any GNN against perturbations of the graph structure
and show that the certified robustness guarantee is tight. 
...
The approach of Jin et al. \cite{jin2020graph} goes in this direction in a way, although they are not considering certifiable robustness.
