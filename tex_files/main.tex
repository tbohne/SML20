...
class of node $t$ and $\mathcal{A}(A)$ defines the set of feasible perturbations.
\begin{gather}
\label{eq:4}
...
datasets \textsc{CORA-ML}, \textsc{CITESEER}, and \textsc{PUBMED} is provided.
...
method. The branch-and-bound method recursively divides the problem into linear sub-problems that can be solved by highly optimized LP-solvers (e.g. CPLEX).
...
and node attributes, which has been proven by numerous publications discussed in section \ref{sec:literature}.
