...
The basic GNN model provides a first step towards incorparating neural networks
into the graph domain, but has several limitations \cite{article} which are tackled in various variants
of GNN of which some will be part of later chapters.\newline

\textbf{Graph Convolutional Networks}\newline

GCNs aim to generalize convolutions to the graph domain. As CNNs have achieved great
success in the area of deep learning, it is intuitive to defnie the convolution
operation on graphs. There are several approaches that can be categorized in
spectral approaches and spatial approaches.\newline

//////////////////////////////////////////////////////

However, there's one big issue which is subject to a rather new branch of research inside the field of GNNs which is the robustness of such models.
There are several publications that analyze the robustness of GNNs to adversarial examples () and recently there came up first approaches to
strengthen the robustness () or even to certify robustness w.r.t. a certain perturbation set.\newline

Some of these works will be discussed in the later chapters of this paper.\newline

\textbf{Why are GNNs or more precisely GCNs of relevance? - Motivation}\newline
...
\begin{figure}[h]
    \includegraphics[width=0.5\textwidth]{img/adversarial_GNN.png}
...
They talk about three categories relevant when talking about the attack model:
\begin{enumerate}
...
\end{enumerate}
Those categories allow more precise considerations of a model's weaknesses and robustness and are later used to compare and interet results of 
different approaches.\newline
\cite{zuegner2019adversarial} confirms previous claims that small graph perturbations consistently lead to a strong decrease in performance for GCNs.\newline
