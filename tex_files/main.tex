...
large number of variables in a GNN makes this approach rather slow. Alternatively, they consider the dual of the LP. Any dual-feasible solution 
...
We denote with $p_{\theta}^t$ the $k$-dimensional vector containing the dual objective function values
...
classes. To facilitate true robustness and not false certainty in the model's predictions, they propose an alternative robust loss that they refer to
as robust hinge loss:
\begin{gather}
...
\ref{eq:2} only improves robustness regarding the labeled nodes, we don't consider the unlabeled nodes. How to handle the semi-supervised setting
...
Overall, the final equation in \ref{eq:3} aims to correctly classify all labeled nodes using the exact GNN, while making sure that every node has at least a
margin of $M_{\ast}$ from the decision boundary even under worst-case perturbations. By setting a smaller margin $M_2$ for the unlabeled nodes,
we can train our classifier to be robust. Since the cross entropy term is unchanged, if the classifier is already robust, the robust hinge
loss is $0$:
...
\end{enumerate}
In the experiments, the labeled nodes are on average more robust than the unlabeled nodes, which is not surprising given that the classifier
