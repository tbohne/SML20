...
A concept, referred to as \textquote{convex envelope} of the objective function, is used to compute lower bounds and additionally, 
...
A further crucial observation is that \textquote{classical} adversarial training procedures based on the idea of adding adversarially
...
In this case, however, not only classification on node level, but also on graph level.
...
the graph level classification assigns a label to the entire graph.
Therefore, the classifier has to be trained on a set of labeled graphs. \cite{Wang_2020} 
An example where such a classification scheme could apply is to assign labels to graphs based on their specific topology
(e.g. circular graphs).
...
and an algorithm that solves it in an efficient manner is introduced.
...
that provides probabilities for the harmfulness of perturbations. The work is a promising starting point for understanding 
the underlying mechanisms of adversarial attacks which could ultimately lead to methods using general attack models 
that are ideally able to recognize incoming attacks based on their specific traits.
