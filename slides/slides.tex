...
\begin{frame}
  ...
  Standard Convolution-Operation kann nicht für Graphen definiert werden!

  Lösung: Embeddings

  Approximierte Graph-Convolution (Kipf et al.):

  -> GCN: $H^{(l+1)} = \sigma [\hat{D}^{- \frac{1}{2}} \hat{A} \hat{D}^{- \frac{1}{2}} H^{(l)} W^{(l)}]$
